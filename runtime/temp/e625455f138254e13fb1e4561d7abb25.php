<?php /*a:3:{s:59:"E:\xiao\smallprogram\application/admin/view\data\index.html";i:1518260525;s:62:"E:\xiao\smallprogram\application/admin/view\public\header.html";i:1518183050;s:62:"E:\xiao\smallprogram\application/admin/view\public\footer.html";i:1518183083;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php if(!(empty($data) || (($data instanceof \think\Collection || $data instanceof \think\Paginator ) && $data->isEmpty()))): if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                <tr>
                                    <td><input class="ids i-checks" checked="chedked" type="checkbox" name="ids[]" value="<?php echo htmlentities($vo['name']); ?>"></td>
                                    <td><?php echo htmlentities($vo['name']); ?></td>
                                    <td>【<?php echo htmlentities($vo['rows']); ?>】 条记录</td>
                                    <td><?php echo htmlentities(format_bytes($vo['data_length'])); ?></td>
                                    <td><?php echo htmlentities($vo['create_time']); ?></td>
                                    <td id="info">等待备份...</td>
                                    <td>
                                        <a class="btn btn-primary btn-xs btn-outline" href="javascript:;" onclick="optimize('<?php echo htmlentities($vo['name']); ?>')">优化表</a>
                                        <a class="btn btn-danger btn-xs btn-outline" href="javascript:;" onclick="repair('<?php echo htmlentities($vo['name']); ?>')">修复表</a>
                                    </td>
                                </tr>
...
function optimize(name){
    $.ajax({
        url : "<?php echo url('Data/optimize'); ?>",
        data : {
            ids : name
        },
        type : 'get',
        ...
        success : function(data){
            layer.msg(data.msg,{icon:1,time:1500,shade: 0.1}, function(index){
                layer.close(index);
                window.location.href="<?php echo url('Data/index'); ?>";
            });
        }
    });
}
