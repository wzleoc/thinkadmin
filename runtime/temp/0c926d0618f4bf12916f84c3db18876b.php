                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php if(!(empty($data) || (($data instanceof \think\Collection || $data instanceof \think\Paginator ) && $data->isEmpty()))): if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                    <tr class="long-td">
                                        <td><?php echo htmlentities(date('Ymd-His',!is_numeric($vo['time'])? strtotime($vo['time']) : $vo['time'])); ?></td>
                                        <td><?php echo htmlentities($vo['part']); ?></td>
                                        <td><?php echo htmlentities($vo['compress']); ?></td>
                                        <td><?php echo htmlentities(format_bytes($vo['size'])); ?></td>
                                        <td><?php echo htmlentities($key); ?></td>
                                        <td>-</td>
                                        <td>
                                            <a class="btn btn-primary btn-xs btn-outline db-import" href="<?php echo url('revert',['time'=>$vo['time']]); ?>">还原</a>
                                            <a class="btn btn-danger btn-xs btn-outline" href="javascript:;" onclick="del(<?php echo htmlentities($vo['time']); ?>)">删除</a>
                                        </td>
                                    </tr>
            ...
            $.ajax({
                ...
                success : function(data){
                    layer.msg(data.msg,{icon:1,time:1500,shade: 0.1}, function(index){
                        layer.close(index);
                        window.location.href="<?php echo url('Data/import'); ?>";
                    });
                }
            });
        })
